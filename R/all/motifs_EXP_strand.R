...
OUTPUT_DIR <- "/Users/mar/BIO/PROJECTS/APOBEC/Project1_TranscriptionLevel/ResultsFinalRall/EXPmotifsStrand"
...
for(j in 1:nrow(motifs)){
  ...
  for(rt in c(0,1,2,3,4,5,6,7)){
    for(strand in c(0,1)){
      trgInitialTemplate <- rbind(trgInitialTemplate, data.table("Motif"=m,"ExpressionBin"=rt,"ExpressionStrand"=strand))
      for(nt in c("A","C","G","T")){
       if(substr(m,2,2) != nt){
        mutInitialTemplate <- rbind(mutInitialTemplate, data.table("Motif"=m,
                                                     "ExpressionBin"=rt, 
                                                     "ExpressionStrand"=strand,
                                                     "MutateAllele"=nt))
       }
      }
    }
...
for(c in 1:nrow(cancers)){
  ...
  for(i in 1:nrow(samples)){
    ...
    mutsG <- muts[,.("Cnt"=sum(Cnt)), by = .(Motif,ExpressionBin,ExpressionStrand,MutateAllele)]
    trgsG <- trgs[,.("TrgCnt"=sum(Cnt)), by = .(Motif,ExpressionBin,ExpressionStrand)]
    
    mutTemplate <- copy(mutInitialTemplate)
    trgTemplate <- copy(trgInitialTemplate)
    
    mutsG <- merge(mutTemplate,mutsG,by=c("Motif","ExpressionBin","ExpressionStrand","MutateAllele"), all.x=TRUE)
    mutsG[is.na(Cnt), Cnt:= 0] 
    trgsG <- merge(trgTemplate,trgsG,by=c("Motif","ExpressionBin","ExpressionStrand"), all.x=TRUE)
    
    data <- merge(trgsG,mutsG,by=c("Motif","ExpressionBin","ExpressionStrand"),all.x = TRUE)
    data$ExpressionStrand <- as.factor(data$ExpressionStrand)
    
    data <- data[ExpressionBin !=0]
    ...
    for(j in 1:nrow(motifs))
    {
      ...
      p <- ggplot(dt, aes(x=ExpressionStrand,y=Cnt,fill=MutateAllele)) + geom_bar(stat="identity") +
        facet_wrap( ~ ExpressionBin, ncol=7) +
        ylab("SBS count") +
        xlab("Transcription strand: coding - 1, template - 0") +
        ggtitle("Low               Gene expression level               High") +
        theme(panel.background = element_blank(),
              axis.title.x = element_text(size=5),
      ...
      p <- ggplot(dt, aes(x=ExpressionStrand,y=MutDensity,fill=MutateAllele)) + geom_bar(stat="identity") +
        facet_wrap( ~ ExpressionBin, ncol=7) +
        ylab("SBS density") +
        xlab("Transcription strand: coding - 1, template - 0") +
        ggtitle("Low               Gene expression level               High") +
      ...
      dtcod <- dt[ExpressionStrand==1,.("Cnt"=sum(Cnt)),by=.(Motif,MotifName,ExpressionBin,TrgCnt)]
      dttem <- dt[ExpressionStrand==0,.("Cnt"=sum(Cnt)),by=.(Motif,MotifName,ExpressionBin,TrgCnt)]
      dtcodtem <- merge(dtcod,dttem,by=c("Motif","MotifName","ExpressionBin"))
      setnames(dtcodtem,c("Motif","MotifName","ExpressionBin","TrgCodingCnt","CodingCnt","TrgTemplateCnt","TemplateCnt"))
      sumCoding <- dtcodtem[,sum(CodingCnt)]
      sumTemplate <- dtcodtem[,sum(TemplateCnt)]
      if(sumCoding < 20 | sumTemplate < 20) {
        log2ratioCodTem <- NA
        log2ratioSlopeRT <- NA
      }else{
        log2ratioCodTem <- dtcodtem[,log2((sum(CodingCnt)/sum(TrgCodingCnt))/(sum(TemplateCnt)/sum(TrgTemplateCnt)))] 
      
        dtcodtem[,log2ratioCodTem := ifelse(CodingCnt==0 | TemplateCnt==0,NA,log2((CodingCnt/TrgCodingCnt)/(TemplateCnt/TrgTemplateCnt))) ]
        motiflm <- lm(log2ratioCodTem ~ ExpressionBin,dtcodtem)
      ...
      # 
       resultsMotif <- rbind(resultsMotif,data.table("Cancer"=cancer,
                                                     ...
                                                     "Motif"=mname,
                                                     "Cnt"=mutcnt,
                                                     "log2ratioCodTem"=log2ratioCodTem,
                                                     "log2ratioSlopeEXP"=log2ratioSlopeRT))
       
    }
    
    # APOBEC signature plots
    dataAPOBEC <- data[substr(Motif,1,2) == "TC" & MutateAllele %in% c("T","G")]
    dataAPOBECgrp <- dataAPOBEC[,.("TrgCnt"=sum(TrgCnt),"Cnt"=sum(Cnt)), by=.(ExpressionBin,ExpressionStrand,MutateAllele)]
    ...
    p <- ggplot(dataAPOBECgrp, aes(x=ExpressionStrand,y=Cnt,fill=MutateAllele)) + geom_bar(stat="identity") +
      facet_wrap( ~ ExpressionBin, ncol=7) +
      ylab("SBS count") +
      xlab("Transcription strand: coding - 1, template - 0") +
      ggtitle("Low               Gene expression level               High") +
    ...
    p <- ggplot(dataAPOBECgrp, aes(x=ExpressionStrand,y=MutDensity,fill=MutateAllele)) + geom_bar(stat="identity") +
      facet_wrap( ~ ExpressionBin, ncol=7) +
      ylab("SBS density") +
      xlab("Transcription strand: coding - 1, template - 0") +
      ggtitle("Low               Gene expression level               High") +
    ...
    dataAPOBEClm <- dataAPOBECgrp[,.("Cnt"=sum(Cnt),"TrgCnt"=max(TrgCnt)),by=.(ExpressionBin,ExpressionStrand)]
    senseData <- dataAPOBEClm[ExpressionStrand == 1]
    senseData[, ExpressionStrand := NULL]
    setnames(senseData,c("ExpressionBin","SenseCnt","SenseTrgCnt"))
    asenseData <- dataAPOBEClm[ExpressionStrand == 0]
    asenseData[, ExpressionStrand := NULL]
    setnames(asenseData,c("ExpressionBin","AsenseCnt","AsenseTrgCnt"))
    strandRatioData <- merge(senseData,asenseData,by=c("ExpressionBin"))
    ...
    dataXCXgrp <- dataXCX[,.("TrgCnt"=sum(TrgCnt),"Cnt"=sum(Cnt)), by=.(ExpressionBin,ExpressionStrand,MutateAllele)]
    ...
    p <- ggplot(dataXCXgrp, aes(x=ExpressionStrand,y=Cnt,fill=MutateAllele)) + geom_bar(stat="identity") +
      facet_wrap( ~ ExpressionBin, ncol=7) +
      ylab("SBS count") +
      xlab("Transcription strand: coding - 1, template - 0") +
      ggtitle("Low               Gene expression level               High") +
    ...
    p <- ggplot(dataXCXgrp, aes(x=ExpressionStrand,y=MutDensity,fill=MutateAllele)) + geom_bar(stat="identity") +
      facet_wrap( ~ ExpressionBin, ncol=7) +
      ylab("SBS density") +
      xlab("Transcription strand: coding - 1, template - 0") +
      ggtitle("Low               Gene expression level               High") +
    ...
    dataXCXlm <- dataXCXgrp[,.("Cnt"=sum(Cnt),"TrgCnt"=max(TrgCnt)),by=.(ExpressionBin,ExpressionStrand)]
    senseData <- dataXCXlm[ExpressionStrand == 1]
    senseData[, ExpressionStrand := NULL]
    setnames(senseData,c("ExpressionBin","SenseCnt","SenseTrgCnt"))
    asenseData <- dataXCXlm[ExpressionStrand == 0]
    asenseData[, ExpressionStrand := NULL]
    setnames(asenseData,c("ExpressionBin","AsenseCnt","AsenseTrgCnt"))
    strandRatioData <- merge(senseData,asenseData,by=c("ExpressionBin"))
    strandRatioData[, Cancer := cancer]
    ...
    p <- ggplot(resultsMotif, aes(x=Motif,y=log2ratioCodTem)) + geom_bar(stat="identity",
                                                                         colour=rgb(38,120,178,maxColorValue = 255),
                                                                         fill=rgb(146,187,216,maxColorValue = 255)) +
      ylab("      Log2-coding/template strand mutation density ratio") +
    ...
    p <- ggplot(resultsMotif, aes(x=Motif,y=log2ratioSlopeEXP)) + geom_bar(stat="identity",
                                                                          colour=rgb(38,120,178,maxColorValue = 255),
                                                                          fill=rgb(146,187,216,maxColorValue = 255)) +
      ylab("Slope of log2-coding/template strand mutation density ratio over gene expression") +
      theme(axis.text.x = element_text(angle = 90, hjust = 1, size=5),
            #axis.text.x = element_blank(),
            axis.text.y = element_text(size=5),
            axis.ticks.x=element_blank(),
...
# write.csv(resultsMwrite.csv(results,paste0(OUTPUT_DIR,"/RTstrandRatio.csv"))
write.csv(results,paste0(OUTPUT_DIR,"/EXPstrandRatio.csv"))
write.csv(resultsXCX,paste0(OUTPUT_DIR,"/EXPstrandRatioXCX.csv"))

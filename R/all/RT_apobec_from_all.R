...
for(i in 1:nrow(cancers))
{
  ...
  for(m in c("TCT","TCA","TCC","TCG"))
  {
    ...
    dt <- tcx[,.("MutationCnt"=sum(MutationCnt)),by=.(RTbin)]
    dt <- dt[RTbin %in% c(0,1,2,3,4,5,6)]
    
    tcxNum <- dataTrg[Motif == m]
    tcxNum <- tcxNum[RTbin %in% c(0,1,2,3,4,5,6)]
    tcxNum <- tcxNum[,.("TargetCnt"=sum(TargetCnt)), by=.(RTbin)]
    dt1 <- merge(dt, tcxNum, by = "RTbin",all.y=TRUE)
   ...
   for(m in C2motifList) 
   {
    ...
    с2oth <- lm(NormalizedDensity ~ RTbin,dt1)
    ...
    oth <- lm(NormalizedDensity ~ RTbin,dt1)

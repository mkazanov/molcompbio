...
MOTIFS_DIR <- "/Users/mar/BIO/PROJECTS/APOBEC/Project1_TranscriptionLevel/Results_CPP/MOTIFS_EXP/"
...
dir.create(file.path(paste0(OUTPUT_DIR,'/EXPstrand'),'RAW'))
dir.create(file.path(paste0(OUTPUT_DIR,'/EXPstrand'),'RATIO'))
...
for(i in 1:nrow(cancers))
{
  ...
  dir.create(file.path(paste0(OUTPUT_DIR,'/EXPstrand/RAW'), cancers[i]$cancer))
  dir.create(file.path(paste0(OUTPUT_DIR,'/EXPstrand/RATIO'), cancers[i]$cancer))
  dir.create(file.path(paste0(OUTPUT_DIR,'/EXPstrand/RAW/',cancers[i]$cancer), "APOBEC"))
  dir.create(file.path(paste0(OUTPUT_DIR,'/EXPstrand/RAW/',cancers[i]$cancer), "OTHER"))
  dir.create(file.path(paste0(OUTPUT_DIR,'/EXPstrand/RATIO/',cancers[i]$cancer), "APOBEC"))
  dir.create(file.path(paste0(OUTPUT_DIR,'/EXPstrand/RATIO/',cancers[i]$cancer), "OTHER"))
}
...
for(i in 1:nrow(cancers)){
  ...
  for (j in 1:nrow(samples)){
    dt <- read.csv(paste0(MOTIFS_DIR,"ALLMUTS_in_EXPbins_",cancers[i]$cancer,"_",samples[j]$sample,".txt"),sep='\t')
    dataMut <- rbind(dataMut,dt)
  }
}

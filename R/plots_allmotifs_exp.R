...
for(s in unique(data$Sample))
{
 ...
 #   StrandTotalRatio = dt2[, sum(LaggingCnt)/(sum(LaggingCnt)+sum(LeadingCnt))]
  ...
  p1 <- ggplot(dtCoef, aes(x = Motif, y = Coef)) + geom_bar(stat="identity") +
  ...
  ggsave(paste0(ROOT_DIR,"plots/",s,".jpg"), plot = p, device = "jpeg", width = 10)
